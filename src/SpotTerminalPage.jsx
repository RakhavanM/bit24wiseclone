...
function ChartPanel({ symbol, timeframe, onTimeframe }) {
  const intervalScale = { '۱د': 0.65, '۵د': 0.82, '۱۵د': 1, '۱س': 1.35, '۴س': 1.8, '۱روز': 2.35 }[timeframe] || 1;
  const candles = useMemo(() => Array.from({ length: 36 }, (_, index) => {
    const wave = Math.sin(index * 0.57 * intervalScale) * symbol.price * 0.004 * intervalScale;
    const trend = index * symbol.price * 0.00017 * intervalScale;
    const close = symbol.price + wave + trend;
    return { close, open: close - Math.cos(index) * symbol.price * .002, high: close + symbol.price * .0034 * intervalScale, low: close - symbol.price * .003 * intervalScale, volume: 25 + ((index * 17) % 42) };
  }), [symbol, timeframe]);
  ...
  const lineKey = `${symbol.code}-${timeframe}`;
  return <section className="spot-chart-panel"><div className="spot-panel-head"><div><span className="spot-panel-kicker"><Activity size={14} /> نمودار قیمت</span><h2>{faNumber(symbol.price, symbol.price < 1 ? 6 : 2)} <small>{symbol.quote}</small></h2></div><div className="spot-chart-tools"><div className="spot-timeframes">{['۱د', '۵د', '۱۵د', '۱س', '۴س', '۱روز'].map((item) => <button className={timeframe === item ? 'active' : ''} type="button" key={item} onClick={() => onTimeframe(item)}>{item}</button>)}</div><button className="spot-icon-button" type="button" aria-label="بزرگ‌نمایی نمودار"><Maximize2 size={16} /></button></div></div><div className="spot-chart-meta"><span className="spot-positive"><TrendingUp size={14} /> +{faNumber(symbol.change * intervalScale, 2)}٪</span><span>بیشترین {faNumber(max, symbol.price < 1 ? 6 : 2)}</span><span>کمترین {faNumber(min, symbol.price < 1 ? 6 : 2)}</span><span>بازه‌ی انتخابی <b>{timeframe}</b></span></div><div className="spot-chart-canvas"><svg viewBox="0 0 660 260" role="img" aria-label={`نمودار قیمت ${symbol.code} در بازه ${timeframe}`} preserveAspectRatio="none"><defs><linearGradient id={`spot-chart-fill-${lineKey.replace(/[^a-z0-9]/gi, '')}`} x1="0" x2="0" y1="0" y2="1"><stop offset="0" stopColor="#0072ff" stopOpacity=".22" /><stop offset="1" stopColor="#0072ff" stopOpacity="0" /></linearGradient></defs>{[38, 86, 134, 182, 230].map((yValue) => <line className="chart-grid-line" key={yValue} x1="0" x2="660" y1={yValue} y2={yValue} />)}<polygon points={`18,230 ${line} 638,230`} fill={`url(#spot-chart-fill-${lineKey.replace(/[^a-z0-9]/gi, '')})`} /><polyline points={line} fill="none" stroke="#0072ff" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />{candles.filter((_, index) => index % 6 === 0).map((candle, index) => <text x={18 + index * 103} y="253" key={index}>{['۱۰:۰۰', '۱۲:۰۰', '۱۴:۰۰', '۱۶:۰۰', '۱۸:۰۰', 'اکنون'][index]}</text>)}</svg></div><div className="spot-chart-disclaimer"><ChartCandlestick size={14} /> داده‌ها شبیه‌سازی‌شده‌اند و صرفاً برای نمایش تجربه‌ی معامله استفاده می‌شوند.</div></section>;
}
...
function OrderForm({ side, setSide, symbol, book, balances, onSubmit, selectedPrice }) {
  ...
  const submit = () => onSubmit({ side, type, price: Number(price) || symbol.price, amount: Number(amount) || 0 });
...
export function SpotTerminalPage() {
  ...
  const [activeHistoryTab, setActiveHistoryTab] = useState('open');
  const [side, setSide] = useState('buy');
  const [notice, setNotice] = useState(null);
  useEffect(() => { setBook(createOrderBook(symbol, Date.now() % 23)); setTrades(createRecentTrades(symbol, Date.now() % 23)); setSide('buy'); setSelectedPrice(''); }, [symbol]);
  ...
  const submitOrder = ({ side: orderSide, type, price, amount }) => {
    if (type === 'market') {
      const executionPrice = orderSide === 'buy' ? book.asks[book.asks.length - 1]?.price : book.bids[0]?.price;
      const result = executeMarketOrder({ side: orderSide, symbol, amount, account, executionPrice });
      if (!result.ok) { setNotice({ type: 'error', text: result.error }); return; }
      setAccount(result.account);
      setActiveHistoryTab('history');
      setNotice({ type: 'success', text: 'سفارش با موفقیت اجرا شد' });
      return;
    }
    const result = placeLimitOrder({ side: orderSide, symbol, price, amount, account });
    if (!result.ok) { setNotice({ type: 'error', text: result.error }); return; }
    setAccount(result.account);
    setActiveHistoryTab('open');
    setNotice({ type: 'success', text: 'سفارش در فهرست سفارش‌های باز قرار گرفت.' });
  };
  const cancelOrder = (orderId) => {
    const result = cancelSimulatedOrder({ account, orderId });
    if (!result.ok) { setNotice({ type: 'error', text: result.error }); return; }
    setAccount(result.account);
    setNotice({ type: 'success', text: 'سفارش لغو شد و موجودی آزاد شد.' });
  };
  const openOrderRows = account.openOrders.map((item) => <div className="spot-history-row" key={item.id}><span>{item.symbol}</span><b className={item.side === 'buy' ? 'spot-positive' : 'spot-negative'}>{item.side === 'buy' ? 'خرید' : 'فروش'}</b><span>{faNumber(item.price, symbol.price < 1 ? 6 : 2)}</span><span>{faNumber(item.amount, 6)}</span><span>{item.type === 'limit' ? 'محدود' : item.type}</span><button className="spot-cancel-order" type="button" onClick={() => cancelOrder(item.id)}>لغو</button></div>);
  const historyRows = account.history.map((item) => <div className="spot-history-row" key={item.id}><span>{item.symbol}</span><b className={item.side === 'buy' ? 'spot-positive' : 'spot-negative'}>{item.side === 'buy' ? 'خرید' : 'فروش'}</b><span>{faNumber(item.price, symbol.price < 1 ? 6 : 2)}</span><span>{faNumber(item.amount, 6)}</span><span>{item.time}</span><span>{item.status === 'filled' ? 'تکمیل‌شده' : item.status}</span></div>);
  return <div className="inner-page spot-terminal-page"><div className="spot-terminal-top page-shell"><div className="spot-terminal-title"><div className="section-kicker"><Activity size={15} /> محیط آزمایشی معامله</div><h1>معاملات اسپات</h1><p>داده‌ها و سفارش‌ها شبیه‌سازی‌شده‌اند؛ تجربه‌ای نزدیک به محیط واقعی معامله.</p></div><div className="spot-terminal-actions"><span className="spot-sim-badge"><i /> شبیه‌سازی فعال</span><button className="spot-reset" type="button" onClick={() => { localStorage.removeItem('bit24-spot-account'); setAccount(defaultSpotAccount); setNotice({ type: 'success', text: 'حساب آزمایشی بازنشانی شد.' }); }}><RefreshCcw size={15} /> بازنشانی حساب</button></div></div><div className="page-shell spot-market-bar"><PairSelector selected={symbol} onChange={setSymbol} /><div className="spot-market-price"><strong>{faNumber(symbol.price, symbol.price < 1 ? 6 : 2)}</strong><span>{symbol.quote}</span><b className={symbol.change >= 0 ? 'spot-positive' : 'spot-negative'}>{symbol.change >= 0 ? '+' : ''}{faNumber(symbol.change, 2)}٪</b></div><div className="spot-market-stats"><span>بیشترین ۲۴ساعت <b>{faNumber(symbol.price * 1.024, symbol.price < 1 ? 6 : 2)}</b></span><span>کمترین ۲۴ساعت <b>{faNumber(symbol.price * .976, symbol.price < 1 ? 6 : 2)}</b></span><span>حجم ۲۴ساعت <b>۱۲۸٫۴M</b></span></div></div><main className="page-shell spot-terminal-grid"><ChartPanel symbol={symbol} timeframe={timeframe} onTimeframe={setTimeframe} /><aside className="spot-side-column"><OrderBook book={book} symbol={symbol} onPrice={(price) => setSelectedPrice(price)} /><RecentTrades trades={trades} symbol={symbol} /></aside><section className="spot-trade-card"><div className="spot-card-head"><h3>ثبت سفارش</h3><span><Clock3 size={14} /> تسویه‌ی شبیه‌سازی‌شده</span></div><OrderForm side={side} setSide={setSide} symbol={symbol} book={book} balances={account.balances} onSubmit={submitOrder} selectedPrice={selectedPrice} /></section><section className="spot-bottom-panel"><div className="spot-bottom-tabs"><button className={activeHistoryTab === 'open' ? 'active' : ''} type="button" onClick={() => setActiveHistoryTab('open')}>سفارش‌های باز ({account.openOrders.length})</button><button className={activeHistoryTab === 'history' ? 'active' : ''} type="button" onClick={() => setActiveHistoryTab('history')}>تاریخچه معاملات ({account.history.length})</button><button className={activeHistoryTab === 'balance' ? 'active' : ''} type="button" onClick={() => setActiveHistoryTab('balance')}>موجودی حساب</button></div>{notice && <div className={`spot-notice ${notice.type}`}>{notice.text}<button type="button" onClick={() => setNotice(null)} aria-label="بستن پیام"><X size={15} /></button></div>}{activeHistoryTab === 'balance' ? <div className="spot-balance-grid">{Object.entries(account.balances).map(([asset, value]) => <div key={asset}><span>{asset}</span><b>{faNumber(value, 6)}</b></div>)}</div> : <div className="spot-history-table">{activeHistoryTab === 'open' ? (openOrderRows.length ? openOrderRows : <div className="spot-empty-state">سفارش بازی در این حساب آزمایشی وجود ندارد.</div>) : (historyRows.length ? historyRows : <div className="spot-empty-state">هنوز معامله‌ای در این حساب آزمایشی ثبت نشده است.</div>)}</div>}</section></main></div>;
}
